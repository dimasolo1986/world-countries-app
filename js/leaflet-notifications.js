L.Control.Notifications = L.Control.extend({
  ...
  onAdd: function (map) {
    ...
    this._container = L.DomUtil.create(
      "div",
      `leaflet-control-notifications ${this.options.className}`
    );
  ...
  info: function (title, message, options) {
    this._add("info", title, message, {
      icon: "fa fa-info-circle",
      ...options,
    });
  },
  warning: function (title, message, options) {
    this._add("warning", title, message, {
      icon: "fa fa-exclamation-triangle",
      ...options,
    });
  },
  success: function (title, message, options) {
    this._add("success", title, message, {
      icon: "fa fa-check-circle",
      ...options,
    });
  },
  alert: function (title, message, options) {
    this._add("alert", title, message, {
  ...
  clear: function () {
    for (var i = 0; i < this._notifications.length; i++) {
      ...
      this._remove(container, this._container);
      i--;
    }
    this._notifications = [];
  },
  _add: function (severity, title, message, options) {
    ...
    var container = L.DomUtil.create(
        "div",
        `leaflet-notification ${severity}`,
        this._container
      ),
      _closable =
        typeof closable !== "undefined" ? closable : this.options.closable,
      _dismissable =
        typeof dismissable !== "undefined"
          ? dismissable
    ...
    contentUtil = L.DomUtil.create(
      "div",
      "leaflet-notification-content",
      container
    );
    titleUtil = L.DomUtil.create(
      "div",
      "leaflet-notification-title",
      contentUtil
    );
    messageUtil = L.DomUtil.create(
      "div",
    ...
    if (_closable) {
      var closeBtnUtil = L.DomUtil.create(
        "span",
      ...
      L.DomEvent.addListener(closeBtnUtil, "click", () =>
        this._remove(container, this._container)
      );
  ...
  _remove: function (container, parent) {
