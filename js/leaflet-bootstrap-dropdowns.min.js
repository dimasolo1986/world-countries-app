...
<svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
    ...
    onAdd: function (map) {
      ...
      this._dropdown = L.DomUtil.create(
        "div",
        "dropdown leaflet-bootstrap-dropdowns"
      );
      if (this.options.className)
        L.DomUtil.addClass(this._dropdown, this.options.className);
      L.DomEvent.disableClickPropagation(this._dropdown);

      this._button = L.DomUtil.create("button", "btn", this._dropdown);
      this._button.type = "button";
      this._button.style.width = "48px";
      this._button.style.height = "48px";
      ...
      this._button.setAttribute(
        "aria-label",
        this.options.ariaLabel ? this.options.ariaLabel : this.options.title
      );
      ...
      for (const item of this.options.menuItems) {
        ...
        if (item.title || item.ariaLabel)
          anchor.setAttribute(
            "aria-label",
            item.ariaLabel ? item.ariaLabel : item.title
          );
